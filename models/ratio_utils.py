...
def get_data(mu_1=0.,mu_2=2.,mu_3=2.,scale_p=0.1,scale_q=0.1,scale_m=1.,mtype='cauchy'):
    ...
    if mtype=='cauchy_mix':
        ...
        m = tfp.distributions.Mixture(
          cat=tfp.distributions.Categorical(probs=[mix, mix, 1-2*mix]),
          components=[
            tfp.distributions.Cauchy(loc=mu_1-1, scale=scale_m),
            tfp.distributions.Cauchy(loc=mu_2+1, scale=scale_m),
            tfp.distributions.Cauchy(loc=2, scale=scale_m)
        ])
    elif mtype=='student':
